    ...
    public void move() {
        /* 
            TODO - write static methods in Item class to return lists of available items.
                 - examples here are default getItems and getCollectibles which just includes loot and levers.
                 - loot and levers can be subclasses of a new class Collectible.
        */
        if (path.isEmpty() || !Item.getItems().contains(item)) {
            // If no collectibles are available, smart thief will move to the nearest.
            if (Item.getCollectibles().isEmpty()) {
                findPath(Door.class);
            } else {
                findPath(Collectible.class);
            }
        }
        coords = path.poll();
    }
